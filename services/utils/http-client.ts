/**
 * Robust HTTP client utility with User-Agent rotation, timeout handling, and redirect support.
 */
...
/**
 * Fetch wrapper with built-in timeout, custom headers, and retry logic
 */
export async function resilientFetch(url: string, options: RequestOptions = {}): Promise<Response> {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      lastError = new Error(isAbort ? `Request timeout after ${timeoutMs}ms for ${url}` : ((err as Error)?.message || 'Fetch failed'))
 ...
 */
export async function resolveFinalUrl(initialUrl: string, timeoutMs = 8000): Promise<string> {
  try {
